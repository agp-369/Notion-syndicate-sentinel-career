...
function DashboardContent() {
  ...
  const runAgenticWorkflow = async () => {
    setIsThinking(true);
    setLastAction(null);
    setCognitiveStage("Agent is querying Sovereign Directory...");
    await new Promise(r => setTimeout(r, 1000));
    setCognitiveStage("AI is reasoning about skills gaps...");
    await new Promise(r => setTimeout(r, 1000));
    ...
    try {
      const res = await fetch("/api/sentinel", {
        ...
        body: JSON.stringify({ mode: "AGENTIC_LOOP", payload: { prompt: "Find match." }, accessToken })
      });
  ...
  return (
    <div className="min-h-screen bg-[#EEF1F9] text-slate-900 font-sans p-4 md:p-8 flex flex-col items-center relative overflow-hidden">
      ...
      {/* --- REBUILT LOCK SCREEN WITH ROBUST TRIGGER --- */}
      {!userId ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center space-y-12 mt-24 max-w-2xl relative z-10">
          ...
          <div className="w-full max-w-xs pt-8">
            <SignInButton mode="modal">
              <button className="w-full py-6 bg-slate-950 text-white rounded-[2.5rem] font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 shadow-2xl active:scale-95 hover:bg-slate-900 transition-all cursor-pointer border-4 border-white/20">
                Initialize Sovereign Node <ArrowRight size={16} />
              </button>
            </SignInButton>
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-6 opacity-60">Identity Handshake Required to Proceed</p>
          </div>
        </motion.div>
      ) : (
        /* --- SOVEREIGN COCKPIT --- */
          ...
          <header className="bg-white/40 backdrop-blur-3xl border border-white/60 p-4 rounded-[2.5rem] shadow-xl flex items-center justify-between">
            <div className="flex items-center gap-4 pl-4">
              ...
              <div className="hidden md:block">
                ...
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Status: {isNotionConnected ? "CONNECTED" : "AUTH_PENDING"}</p>
              </div>
            </div>
            <nav className="flex bg-white/20 p-1.5 rounded-[2rem] gap-1">
              {["COMPASS", "AUDIT", "HISTORY"].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? "bg-white text-indigo-600 shadow-xl" : "text-slate-500"}`}>{tab}</button>
              ))}
            </nav>
              ...
              {activeTab === "COMPASS" && (
                <motion.div key="compass" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="grid grid-cols-1 md:grid-cols-12 gap-8">
                  <div className="md:col-span-5 bg-white/40 backdrop-blur-3xl border border-white/60 p-10 rounded-[4rem] shadow-2xl space-y-8">
                    ...
                    {!isNotionConnected ? (
                      ...
                      <div className="p-8 bg-emerald-500/10 border-2 border-emerald-500/20 rounded-[2.5rem] flex flex-col items-center gap-4 text-center">
                        <CheckCircle2 size={40} className="text-emerald-500" /><p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Verified</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className={`md:col-span-7 p-12 bg-white/60 backdrop-blur-3xl border border-white/80 rounded-[4.5rem] shadow-3xl flex flex-col justify-between transition-opacity ${!isNotionConnected ? "opacity-20 pointer-events-none" : "opacity-100"}`}>
                    ...
                    <div className="flex-1 space-y-6">
                      {isThinking ? (
                        ...
                        <div className="space-y-6">
                          <div className="p-6 bg-emerald-500/10 border border-emerald-500/20 rounded-[2rem] text-left"><p className="text-sm font-medium text-slate-700 italic">"Autonomous Match Identified via MCP Tools."</p></div>
                          <button className="w-full py-6 bg-emerald-500 text-white rounded-[2.5rem] font-black uppercase tracking-widest text-xs active:scale-95 shadow-xl transition-all">Approve & Syndicate</button>
                        </div>
                      ) : (
                    ...
                    {!lastResult && !isThinking && <button onClick={runAgenticWorkflow} className="mt-8 w-full py-8 bg-indigo-600 text-white rounded-[3rem] font-black uppercase tracking-widest text-xs active:scale-95 transition-all shadow-2xl shadow-indigo-200">Synthesize Talent Match</button>}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          </main>
          <footer className="text-center pt-8 opacity-30"><p className="text-[9px] font-black text-slate-950 uppercase tracking-[1.2em]">Syndicate Sentinel // Sovereign OS v9.5</p></footer>
        </div>
      )}
    </div>
  );
