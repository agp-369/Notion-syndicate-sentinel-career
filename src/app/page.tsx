...
export default function Home() {
  ...
  // 🤝 THE AGENTIC ORCHESTRATION (Multi-Step Logic)
  const runAgenticWorkflow = async () => {
    ...
    try {
      const res = await fetch("/api/sentinel", {
        ...
        body: JSON.stringify({ 
          mode: "AGENTIC_LOOP", 
          payload: { prompt: "Match senior/junior and create syllabus." },
          accessToken 
        })
      });
    ...
    <div className="min-h-screen bg-[#EEF1F9] text-slate-900 font-sans p-4 md:p-8 flex flex-col items-center relative overflow-hidden">
      ...
      {!userId ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center space-y-12 mt-24 max-w-2xl relative z-10">
          <div className="w-28 h-28 bg-white/40 backdrop-blur-2xl border border-white/60 rounded-[3rem] flex items-center justify-center shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs font-bold text-indigo-600 uppercase tracking-[0.6em]">Enterprise Career OS // MCP v1.0</p>
          </div>
          <div className="w-full max-w-xs pt-8">
            <div className="w-full py-6 bg-slate-950 text-white rounded-[2.5rem] font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 shadow-2xl active:scale-95 transition-all cursor-pointer border-4 border-white/20">
        ...
        <div className="max-w-6xl w-full space-y-8 relative z-10">
              ...
              {activeTab === "COMPASS" && (
                <motion.div key="compass" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="grid grid-cols-1 md:grid-cols-12 gap-8">
                  ...
                  <div className="md:col-span-5 bg-white/40 backdrop-blur-3xl border border-white/60 p-10 rounded-[4rem] shadow-2xl space-y-8">
                    ...
                    {!isNotionConnected ? (
                      <a href="/api/notion/auth" className="w-full py-6 bg-slate-950 text-white rounded-[2rem] font-black uppercase tracking-widest text-[10px] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
                        <Zap size={16} fill="currentColor" /> Authorize with Notion
                      </a>
                    ) : (
                      <div className="p-8 bg-emerald-500/10 border-2 border-emerald-500/20 rounded-[2.5rem] flex flex-col items-center gap-4 text-center">
                        ...
                        <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Handshake Verified</p>
                        <p className="text-xs font-medium text-slate-500 italic">"Access Token Bound to Local Session"</p>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="flex justify-between items-start border-b border-slate-200 pb-8 mb-8">
                      <div className="space-y-4 text-left">
                        <div className="flex items-center gap-3 text-indigo-600 font-black text-[10px] uppercase tracking-widest">
                          <Activity size={16} /> Reasoning Log
                        </div>
                      ...
                      {isThinking ? (
                        <div className="space-y-4">
                          <div className="flex items-center gap-3 text-indigo-600 animate-pulse">
                            ...
                            <span className="text-xs font-bold uppercase tracking-widest">{cognitiveStage}</span>
                          </div>
                          <div className="bg-slate-100 p-6 rounded-[2rem] border border-slate-200">
                            <p className="text-[10px] font-mono text-slate-500">Searching directory for skills gap...</p>
                          </div>
                        </div>
                      ) : lastResult ? (
                        <div className="space-y-6">
                          <div className="p-6 bg-emerald-500/10 border border-emerald-500/20 rounded-[2rem] space-y-2">
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Decision Proposed</p>
                            <p className="text-sm font-medium text-slate-700 italic">"Perfect match found. Senior React Developer paired with Junior Node. Workspace ready for syndication."</p>
                          </div>
                          <button className="w-full py-6 bg-emerald-500 text-white rounded-[2.5rem] font-black uppercase tracking-widest text-xs shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">
                            <Zap size={18} fill="currentColor" /> Approve & Syndicate to Notion
                          </button>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center py-12 opacity-30 text-slate-400 space-y-4">
                          <Terminal size={48} strokeWidth={1} />
                          <p className="text-[10px] font-black uppercase tracking-widest">Ready for Command</p>
                        </div>
                      )}
                    </div>

                    {!lastResult && !isThinking && (
                      <button onClick={runAgenticWorkflow} className="mt-8 w-full py-8 bg-indigo-600 text-white rounded-[3rem] font-black uppercase tracking-widest text-xs shadow-[0_20px_50px_rgba(79,70,229,0.3)] hover:scale-[1.02] active:scale-95 transition-all">
                        Synthesize Talent Match
                      </button>
                    )}
                  </div>
                </motion.div>
          ...
          <footer className="text-center pt-8 opacity-30">
            <p className="text-[9px] font-black text-slate-950 uppercase tracking-[1.2em]">Syndicate Sentinel // Sovereign OS v9.0 Strict MCP</p>
          </footer>
        </div>
      )}
    </div>
